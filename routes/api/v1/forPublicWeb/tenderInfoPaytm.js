...
async function paymentsucMSG(pMobile,pEmail,response){
  ...
  // /api/v1/forPublicWeb/tenderInfoPaytm/payment
...
console.log(req.params.id)
    ...
    async function payIt(req,res,allData,myUser)
    {
        ...
        let params ={}
        params['CALLBACK_URL'] = `${keys.localBackend}/api/v1/forPublicWeb/tenderInfoPaytm/callback/${allTranId}`,
        ...
        checksum_lib.genchecksum(params,Key,function(err,checksum){
            ...
            var html = '<html><body><center><h1>Please wait! Do not refresh the page -- Mera Tender</h1></center><form method="post" action="'+txn_url+'" name="f1">'+form_fields +'</form><script type="text/javascript">document.f1.submit()</script></body></html>'
...
// http://localhost:2040/api/v1/forPublicWeb/tenderInfoPaytm/callback/62ff484e34c0e8ac21f96863
    router.post("/callback/:allTranId",  (req, res) => {
        const allTranId = req.params.allTranId
   console.log("callback url got called")
...
async function getPrice(req,res,pri,tenderId){ 
   ...
   dataToAdd.type = pri.period
   dataToAdd.period = pri.type
   dataToAdd.tenderId = tenderId
...
async function sendNewLink(req,res,allTran){
    ...
    res.status(200).json(keys.localBackend+ `/api/v1/forPublicWeb/tenderInfoPaytm/payment/${id1}/${allTranId}`);
}

router.post("/pay", passport.authenticate("jwt",{session: false}),async(req,res) => {
      console.log(req.body)
        await Price.findOne({type:"contact"}).then(pri => {
            if(pri) {
                getPrice(req,res,pri,req.body.tenderId)
            } else {
