...
export default function OneBusDataCom({businessLink}) {
    ...
    const getBusinessData = async() => {
        await axios
                .get(`/api/v1/addition/addBusiness/getLink/${businessLink}`)
	...
	return (
		<div className={classes.root}>
			<main className={classes.content}>
				...
				<Paper>
                <div className={classes.profileTop} style={{ marginTop:"20px" }}>
				<Typography align="center" gutterBottom variant="h4" color="primary">
					{allData.businessName }
				</Typography>
			<Avatar  sx={{ width: 100, height: 100 }} 
			alt={allData.businessName} className={classes.profileImg} 
			src={"https://i.pinimg.com/736x/7c/d7/81/7cd781a7a29b266d6efa30b01cd3a238.jpg"} />
					
			</div>

      <Typography variant="body2" gutterBottom style={{textAlign:"center", marginLeft:"1.2rem", marginRight:"1.2rem"}}>
        <q>{allData.shortDescription}</q>
      </Typography>

                <Grid container spacing={2} style={{ marginTop:"1px" }}>
        {/* iTEM sTART */}
       {allData?.businessName && (<><Grid item xs={5} md={4}>
           <Item1>Business Names</Item1>
         </Grid>
         <Grid item xs={7} md={8}>
           <Item2>{allData.businessName}</Item2>
         </Grid></>)}
        {/* iTEM eND */}

        {/* iTEM sTART */}
       {allData?.mobileNo && (<><Grid item xs={5} md={4}>
        ...
        {/* iTEM sTART */}
       {allData?.whatsAppNo && (<><Grid item xs={5} md={4}>
           <Item1>WhatsApp</Item1>
         </Grid>
         <Grid item xs={7} md={8}>
           <Item2>{allData.whatsAppNo}</Item2>
         </Grid></>)}
        {/* iTEM eND */}

        {/* iTEM sTART */}
       {allData?.emailId && (<><Grid item xs={5} md={4}>
           <Item1>Email ID</Item1>
         </Grid>
         <Grid item xs={7} md={8}>
           <Item2>{allData.emailId}</Item2>
         </Grid></>)}
        {/* iTEM eND */}

        {/* iTEM sTART */}
       {allData?.website && (<><Grid item xs={5} md={4}>
           <Item1>Website</Item1>
         </Grid>
         <Grid item xs={7} md={8}>
           <Item2>{allData.website}</Item2>
         </Grid></>)}
        {/* iTEM eND */}

        {/* iTEM sTART */}
       {allData?.gstNumber && (<><Grid item xs={5} md={4}>
           <Item1>Gst Number</Item1>
         </Grid>
         <Grid item xs={7} md={8}>
           <Item2>{allData.gstNumber}</Item2>
         </Grid></>)}
        {/* iTEM eND */}

        {/* iTEM sTART */}
       {allData?.district?.districtLink && (<><Grid item xs={5} md={4}>
           <Item1>District</Item1>
         </Grid>
         <Grid item xs={7} md={8}>
           <Item2>{allData.district?.districtName}</Item2>
         </Grid></>)}
        {/* iTEM eND */}

        {/* iTEM sTART */}
       {allData?.state?.stateLink && (<><Grid item xs={5} md={4}>
           <Item1>State</Item1>
         </Grid>
         <Grid item xs={7} md={8}>
           <Item2>{allData.state.stateName}</Item2>
         </Grid></>)}
        {/* iTEM eND */}

        {/* iTEM sTART */}
       {allData?.pinCode && (<><Grid item xs={5} md={4}>
           <Item1>PinCode</Item1>
         </Grid>
         <Grid item xs={7} md={8}>
           <Item2>{allData.pinCode}</Item2>
         </Grid></>)}
        {/* iTEM eND */}
        {/* iTEM sTART */}
       {allData?.fullAddress && (<><Grid item xs={5} md={4}>
           <Item1>Full Address</Item1>
         </Grid>
         <Grid item xs={7} md={8}>
           <Item2>{allData.fullAddress}</Item2>
         </Grid></>)}
        {/* iTEM eND */}
        {/* iTEM sTART */}
       {allData?.fullDescription && (<><Grid item xs={5} md={4}>
           <Item1>Full Description</Item1>
         </Grid>
         <Grid item xs={7} md={8}>
           <Item2>{allData.fullDescription}</Item2>
         </Grid></>)}
        {/* iTEM eND */}
